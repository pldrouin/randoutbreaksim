...
inline static uint32_t gen_att_infpop_gauss(sim_vars* sv){double ret; do {ret=sv->pars.mu+gsl_ran_gaussian_ziggurat(sv->r,sv->pars.sigma);} while(ret<2); return (uint32_t)ret;}
...
inline static void gen_att_inf_infpop_pinf1_gauss(sim_vars* sv){double ret; do {ret=sv->pars.mu+gsl_ran_gaussian_ziggurat(sv->r,sv->pars.sigma);} while(ret<2); sv->curii->nattendees=(uint32_t)ret; sv->curii->ninfections=sv->curii->nattendees-1;}
...
inline static void gen_att_inf_infpop_gauss(sim_vars* sv){double ret; do {ret=sv->pars.mu+gsl_ran_gaussian_ziggurat(sv->r,sv->pars.sigma);} while(ret<2); sv->curii->nattendees=(uint32_t)ret; sv->curii->ninfections=gsl_ran_binomial(sv->r, sv->pars.pinf, sv->curii->nattendees-1);;}
