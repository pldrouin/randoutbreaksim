...
\defleftcol{Infectious phase} & Begins when an individual is exposed, and ends when the individual is infectious.\\
...
{\raggedright Probability of testing positive\\for non-self-isolating individuals} & \texttt{ttpr} & 70\%\\
{\raggedright Probability of testing positive\\for self-isolating individuals} & \texttt{mtpr} & 70\%\\
Testing delay & \texttt{tdeltat} & 2 days\\
\hline
\multicolumn{3}{c}{Contact tracing parameters} \\
Probability of being contacted & \Pt & 80\%\\
Historical contact window & \texttt{ctwindow} & 2 days\\
\hline
\end{tabular}
\end{table}
...
\item The latent phase was set by difference between the self isolating individuals incubation phase (5.5 days) and infectious phase (2.2 days); therefore, the mean was 3.3 days. To match the \nfpc\ percentile of the combined latent and pre-symptomatic infectious phases with the \nfpc\ percentile of the non-self-isolating incubation phase as closely as possible, the \nfpc\ percentile of the latent phase was also set to 3.3 days. Thus, the duration of the latent phase is assumed fixed.  
...
But what happens to outbreaks if \Rz\ and \Reff\ are greater than 1? Although the exact value of the COVID-19 \Rz\ is debatable [REF,REF...], it is consensus that this value is well above 1; therefore, measures need to be put in place to prevent the spread of the virus. 
